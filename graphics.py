...
def main():
    window = gr.GraphWin("House", 650, 650)
    draw_image(window)
    window.getMouse()


def draw_image(window):
    house_x, house_y = window.width // 2, window.height * 1.5 // 2
    house_width = window.width // 2
...
def draw_house_window(window, x, y, width, height):
    ...
    line1.draw(window)
    line2.draw(window)
    line1.setOutline("goldenrod")
    line2.setOutline("goldenrod")
    line1.setWidth(1)
    line2.setWidth(1)
